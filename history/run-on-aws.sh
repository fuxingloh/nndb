...
# Run from the repo root ON THE BOX:  bash history/run-on-aws.sh [label]
#
...
echo "================ dataset ================"
if [ ! -f "$ROOT/database/data/sift/sift_base.fvecs" ]; then
  bash "$ROOT/database/scripts/download-sift.sh"
fi
...
echo "================ batch sweep ================"
bash "$ROOT/history/measure-batch.sh" "history/004-${LABEL}-baseline.json" "$LABEL"

echo "================ done -> history/004-${LABEL}-baseline.json ================"
