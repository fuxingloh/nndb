...
# JSON results under notes/hw/.
#
# Usage:
#   bash notes/sweep-hardware.sh \
...
one() {
  ...
  rsync -az --delete \
    --exclude 'nndb/data' --exclude 'nndb/target' --exclude 'web/node_modules' \
  ...
  ssh "${SSHOPTS[@]}" "${SSH_USER}@${ip}" \
    "cd ~/vps && bash notes/run-cohere-bench.sh '$label'" 2>&1 \
    | tee "$OUT/${label}.log" \
    | grep -E '^(EXACT_JSON|FUNNEL_JSON)=' \
    | sed -E 's/^(EXACT|FUNNEL)_JSON=//' > "$OUT/${label}.jsonl"
  echo "saved -> notes/hw/${label}.jsonl ($(wc -l < "$OUT/${label}.jsonl") lines)"
}
